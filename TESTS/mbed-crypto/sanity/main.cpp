...
void test_crypto_asymmetric_encrypt_decrypt(void)
{
    ...
    TEST_ASSERT_EQUAL(PSA_SUCCESS, psa_allocate_key(key_type, key_bits, &key_handle));
...
void test_crypto_symmetric_cipher_encrypt_decrypt(void)
{
    ...
    TEST_ASSERT_EQUAL(PSA_SUCCESS, psa_allocate_key(key_type, PSA_BYTES_TO_BITS(sizeof(key)), &key_handle));
...
void test_crypto_key_derivation(void)
{
    ...
    TEST_ASSERT_EQUAL(PSA_SUCCESS, psa_allocate_key(derived_key_type, derived_key_bits, &derived_key_handle));
...
void test_crypto_key_handles(void)
{
    ...
    key_handle = 0;
    TEST_ASSERT_EQUAL(PSA_SUCCESS, psa_allocate_key(type, bits, &key_handle));
    ...
    key_handle = 0;
    TEST_ASSERT_EQUAL(PSA_SUCCESS, psa_create_key(PSA_KEY_LIFETIME_PERSISTENT, id, type, bits, &key_handle));
    TEST_ASSERT_NOT_EQUAL(0, key_handle);
    psa_key_policy_init(&policy);
    psa_key_policy_set_usage(&policy, usage, alg);
    TEST_ASSERT_EQUAL(PSA_SUCCESS, psa_set_key_policy(key_handle, &policy));
