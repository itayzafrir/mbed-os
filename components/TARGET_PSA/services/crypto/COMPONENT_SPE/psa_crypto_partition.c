        ...
        case PSA_IPC_CALL: {
            ...
            switch (psa_key_mng.func) {
                ...
                case PSA_ALLOCATE_KEY: {
                    ...
                    bytes_read = psa_read(msg.handle, 1, &max_bits, msg.in_size[1]);
                    if (bytes_read != msg.in_size[1]) {
                        SPM_PANIC("SPM read length mismatch");
                    }

                    status = psa_allocate_key(psa_key_mng.type, max_bits, &psa_key_mng.handle);
                    if (status == PSA_SUCCESS) {
                ...
                case PSA_CREATE_KEY: {
                    ...
                    status = psa_create_key(psa_key_mng.lifetime, id, psa_key_mng.type, max_bits, &psa_key_mng.handle);
                    if (status == PSA_SUCCESS) {
                        psa_write(msg.handle, 0, &psa_key_mng.handle, sizeof(psa_key_mng.handle));
                    }
                    break;
